...
> (see [§5 Challenges](#5--challenges--lessons-learned----the-overfitting-problem)).
...
### How It Works — Forward & Backward Propagation in Detail
...
Direct convolution with nested loops is too slow in pure Python.  Instead we use the **im2col** trick:
...
3. **Matrix multiply** — reshape the filter bank $W \in \mathbb{R}^{F \times f_h \times f_w \times C}$ into a matrix $W_{\text{col}} \in \mathbb{R}^{(f_h f_w C) \times F}$, then:
   ...
   $$Z_{\text{col}} = X_{\text{col}} \, W_{\text{col}} + b \qquad
     b \in \mathbb{R}^{F}$$
...
**col2im** is the exact inverse of im2col: it takes each row of $\mathrm{d}X_{\text{col}}$, reshapes it to $(f_h, f_w, C)$, and *adds* it to the correct spatial location in the (padded) gradient map.  Regions that are visited multiple times (when $S < f_h$) accumulate gradients.  After filling the padded array, the padding strip is sliced off to recover the original $(H, W)$ dimensions.
...
The gradient is passed through unchanged where $Z > 0$, and zeroed out everywhere else.  This is an element-wise operation — shapes are preserved.
...
While computing the max, we record the **argmax position** inside each window in a binary mask $M$ (same shape as $A_{\text{prev}}$) where $M = 1$ at every winning pixel and $0$ elsewhere.
...
This is the standard "max-pooling switch" technique: only the neuron that fired in the forward pass receives gradient in the backward pass.
...
The transpose puts features along rows and examples along columns — this matches the convention used by the Dense layers.
...
$$Z = W A_{\text{prev}} + b, \qquad
  W \in \mathbb{R}^{n^{[l]} \times n^{[l-1]}},\;
  b \in \mathbb{R}^{n^{[l]} \times 1}$$
...
#### 4.6  Loss Function & Initial Gradient

We use **binary cross-entropy** for this two-class problem:
...
From here, the chain rule propagates this gradient backward through every layer in **reverse architecture order** — Dense → Flatten → MaxPool → ReLU → Conv2D — each layer computing $\frac{\partial \mathcal{L}}{\partial A_{\text{prev}}}$ for the layer before it and $\frac{\partial \mathcal{L}}{\partial W}, \frac{\partial \mathcal{L}}{\partial b}$ for its own parameters.
...
### 5.  Challenges & Lessons Learned  —  The Overfitting Problem
...
**What worked**: The CNN learned spatial hierarchies — the conv filters detected edges and textures that the dense network missed.  Test accuracy improved from 80% → 84%.
...
**What we tried**: Cut conv filters by 50%, shrunk dense layers from 256/128 → 64/32, added inverted dropout (keep_prob=0.5) after each dense layer, and applied AdamW-style decoupled weight decay at $\lambda = 10^{-4}$.

**Why it failed**: The regularisation was **too aggressive for the model capacity that remained**.

- **Dropout at 0.5 on 64-unit and 32-unit layers** left only 32 and 16 active neurons per forward pass — too few to form useful representations.  Dropout is designed for large fully-connected layers (≥256 units); on small layers it destroys signal faster than it prevents co-adaptation.
...
- **Combined effect**: the model was both under-parameterised (can't learn enough) *and* over-regularised (can't use what little it learned).  It converged to a degenerate solution that memorised the training set through a narrow set of brittle features that didn't generalise at all.
...
- **Reduced dense layers** from 256/128 → 128/64 — focused parameter reduction where overfitting is worst (the FC layers hold the majority of parameters: $128 \times 4096 = 524\text{K}$ vs the original $256 \times 8192 = 2.1\text{M}$)
...
**What we tried**: Instead of on-the-fly flips, we pre-generated 4 variants of every training image (original, flipped, rotated + brightness-jittered, zoom-cropped + flipped), expanding the training set from 209 → 836 images.
...
#### 5.6  Root Cause Analysis — Why 100% Train Accuracy Persists

After four iterations of regularisation, the training accuracy remains pinned at 100% while test accuracy hovers around 84–86%.  This is not a failure of the regularisation techniques — it's a **fundamental data problem**:
...
3. **The images are homogeneous** — same resolution (64×64), same lighting, same framing, same background.  There's limited diversity for the model to learn invariance from.
...
> **Note on test accuracy**: The test set contains only 50 images.  With a sample this small, accuracy measurements have a ~±10% confidence interval.  The 84–86% range represents the model's true generalisation ceiling given the dataset size — see [§5 Challenges & Lessons Learned](#5--challenges--lessons-learned----the-overfitting-problem) for the full story.  The model reliably distinguishes cats from non-cats in practice, as verified by manual testing with diverse real-world images.
